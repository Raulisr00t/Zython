...
fn dumpNode(node: *const ast.Node, writer: anytype, depth: usize) !void {
    ...
    switch (node.*) {
        ...
        .tuple_lit => |t| {
            try writer.print("TupleLit\n", .{});
            for (t.elements) |e| try dumpNode(e, writer, depth + 1);
        },
        .dict_lit => |d| {
            try writer.print("DictLit\n", .{});
            for (d.keys, d.values) |k, v| {
                try dumpNode(k, writer, depth + 1);
                try dumpNode(v, writer, depth + 1);
            }
        },
        .subscript => |s| {
        ...
        .attribute => |a| {
            try writer.print("Attribute {s}\n", .{a.attr});
            try dumpNode(a.obj, writer, depth + 1);
        },
        .class_def => |c| {
            try writer.print("ClassDef {s}({s})\n", .{ c.name, c.base orelse "" });
            for (c.body) |stmt| try dumpNode(stmt, writer, depth + 1);
        },
        .try_stmt => |t| {
            try writer.print("Try\n", .{});
            for (t.body) |stmt| try dumpNode(stmt, writer, depth + 1);
            for (t.handlers) |h| {
                try dumpIndent(writer, depth);
                try writer.print("Except {s} as {s}\n", .{ h.exc_type orelse "*", h.name orelse "" });
                for (h.body) |stmt| try dumpNode(stmt, writer, depth + 1);
            }
            if (t.finally_body.len > 0) {
                try dumpIndent(writer, depth);
                try writer.print("Finally\n", .{});
                for (t.finally_body) |stmt| try dumpNode(stmt, writer, depth + 1);
            }
        },
        .raise_stmt => |r| {
            try writer.print("Raise\n", .{});
            if (r.exc) |e| try dumpNode(e, writer, depth + 1);
        },
    }
}
