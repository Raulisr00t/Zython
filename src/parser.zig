...
pub const Parser = struct {
    ...
    fn parseStmt(self: *Parser) anyerror!*ast.Node {
        ...
        if (self.at(.kw_class)) return self.parseClassDef();
        if (self.at(.kw_try)) return self.parseTry();
        ...
        if (self.at(.kw_raise)) {
            ...
            if (!self.at(.newline)) exc = try self.parseExpr();
            _ = try self.expect(.newline);
            return self.newNode(.{ .raise_stmt = .{ .exc = exc } });
        }
    ...
    fn validateAssignTarget(self: *Parser, node: *ast.Node) !void {
        if (node.* != .name and node.* != .subscript and node.* != .attribute) {
    ...
    fn parseClassDef(self: *Parser) !*ast.Node {
        _ = try self.expect(.kw_class);
        const name = (try self.expect(.name)).value.text;
        var base: ?[]const u8 = null;
        if (self.at(.lparen)) {
            _ = self.advance();
            if (!self.at(.rparen)) base = (try self.expect(.name)).value.text;
            _ = try self.expect(.rparen);
        }
        _ = try self.expect(.colon);
        const body = try self.parseBlock();
        return self.newNode(.{ .class_def = .{ .name = name, .base = base, .body = body } });
    }

    fn parseTry(self: *Parser) !*ast.Node {
        _ = try self.expect(.kw_try);
        _ = try self.expect(.colon);
        const body = try self.parseBlock();

        var handlers: std.ArrayList(ast.ExceptHandler) = .empty;
        while (self.at(.kw_except)) {
            _ = self.advance();
            var exc_type: ?[]const u8 = null;
            var name: ?[]const u8 = null;
            if (!self.at(.colon)) {
                exc_type = (try self.expect(.name)).value.text;
                if (self.at(.kw_as)) {
                    _ = self.advance();
                    name = (try self.expect(.name)).value.text;
                }
            }
            _ = try self.expect(.colon);
            const handler_body = try self.parseBlock();
            try handlers.append(self.allocator, .{ .exc_type = exc_type, .name = name, .body = handler_body });
        }

        var finally_body: []const *ast.Node = &.{};
        if (self.at(.kw_finally)) {
            _ = self.advance();
            _ = try self.expect(.colon);
            finally_body = try self.parseBlock();
        }

        if (handlers.items.len == 0 and finally_body.len == 0) {
            return self.fail("try statement must have at least one 'except' or 'finally' clause", .{}, ParseError.InvalidTryStatement);
        }

        return self.newNode(.{ .try_stmt = .{
            .body = body,
    ...
    fn parsePostfix(self: *Parser) anyerror!*ast.Node {
        ...
        while (true) {
            if (self.at(.lparen)) {
                ...
                _ = self.advance();
                const name = (try self.expect(.name)).value.text;
                node = try self.newNode(.{ .attribute = .{ .obj = node, .attr = name } });
            } else {
                break;
            }
        }
    ...
    fn parseAtom(self: *Parser) anyerror!*ast.Node {
        ...
        switch (tok.type) {
            ...
            .lparen => {
                ...
                if (self.at(.rparen)) {
                    _ = self.advance();
                    return self.newNode(.{ .tuple_lit = .{ .elements = &.{} } });
                }
                ...
                if (self.at(.comma)) {
                    ...
                    _ = try self.expect(.rparen);
                    return self.newNode(.{ .tuple_lit = .{ .elements = try elements.toOwnedSlice(self.allocator) } });
                }
                _ = try self.expect(.rparen);
                return first;
            },
            ...
            .lbrace => {
                ...
                if (!self.at(.rbrace)) {
                    ...
                    while (self.at(.comma)) {
                        ...
                        const k2 = try self.parseExpr();
                        _ = try self.expect(.colon);
                        try keys.append(self.allocator, k2);
                        try values.append(self.allocator, try self.parseExpr());
                    }
                }
                _ = try self.expect(.rbrace);
                return self.newNode(.{ .dict_lit = .{
...
test "tuple literal vs grouping" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();

    const m1 = try parseSource(a, "x = (1)\n");
    try std.testing.expect(m1.module.body[0].assign.value.* == .num_int);

    const m2 = try parseSource(a, "x = (1,)\n");
    try std.testing.expect(m2.module.body[0].assign.value.* == .tuple_lit);
    try std.testing.expectEqual(@as(usize, 1), m2.module.body[0].assign.value.tuple_lit.elements.len);

    const m3 = try parseSource(a, "x = (1, 2, 3)\n");
    try std.testing.expectEqual(@as(usize, 3), m3.module.body[0].assign.value.tuple_lit.elements.len);

    const m4 = try parseSource(a, "x = ()\n");
    try std.testing.expectEqual(@as(usize, 0), m4.module.body[0].assign.value.tuple_lit.elements.len);
}
...
test "dict literal" {
    ...
    const module = try parseSource(a, "d = {\"a\": 1, \"b\": 2}\n");
    const value = module.module.body[0].assign.value;
    try std.testing.expect(value.* == .dict_lit);
    try std.testing.expectEqual(@as(usize, 2), value.dict_lit.keys.len);
    try std.testing.expectEqualStrings("a", value.dict_lit.keys[0].str);
    try std.testing.expectEqual(@as(i64, 1), value.dict_lit.values[0].num_int);
}
...
test "attribute access and assignment" {
    ...
    const module = try parseSource(a, "y = self.x\nself.x = 5\n");
    const load = module.module.body[0].assign.value;
    try std.testing.expect(load.* == .attribute);
    try std.testing.expectEqualStrings("self", load.attribute.obj.name);
    try std.testing.expectEqualStrings("x", load.attribute.attr);

    const store = module.module.body[1];
    try std.testing.expect(store.assign.target.* == .attribute);
}
...
test "method call syntax" {
    ...
    const module = try parseSource(a, "obj.method(1, 2)\n");
    const call = module.module.body[0].expr_stmt.value;
    try std.testing.expect(call.* == .call);
    try std.testing.expect(call.call.func.* == .attribute);
    try std.testing.expectEqualStrings("method", call.call.func.attribute.attr);
}

test "classdef no base" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();

    const module = try parseSource(a, "class Foo:\n    def bar(self):\n        return 1\n");
    const node = module.module.body[0];
    try std.testing.expect(node.* == .class_def);
    try std.testing.expectEqualStrings("Foo", node.class_def.name);
    try std.testing.expect(node.class_def.base == null);
    try std.testing.expect(node.class_def.body[0].* == .function_def);
}

test "classdef with base" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();

    const module = try parseSource(a, "class Dog(Animal):\n    x = 1\n");
    const node = module.module.body[0];
    try std.testing.expectEqualStrings("Dog", node.class_def.name);
    try std.testing.expectEqualStrings("Animal", node.class_def.base.?);
}
...
test "try except finally" {
    ...
    const module = try parseSource(
        a,
        "try:\n    x = 1\nexcept ValueError as e:\n    x = 2\nexcept TypeError:\n    x = 3\nfinally:\n    x = 4\n",
    );
    const node = module.module.body[0];
    try std.testing.expect(node.* == .try_stmt);
    try std.testing.expectEqual(@as(usize, 2), node.try_stmt.handlers.len);
    try std.testing.expectEqualStrings("ValueError", node.try_stmt.handlers[0].exc_type.?);
    try std.testing.expectEqualStrings("e", node.try_stmt.handlers[0].name.?);
    try std.testing.expectEqualStrings("TypeError", node.try_stmt.handlers[1].exc_type.?);
    try std.testing.expect(node.try_stmt.handlers[1].name == null);
    try std.testing.expectEqual(@as(usize, 1), node.try_stmt.finally_body.len);
}

test "bare except and try without except or finally" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();

    const module = try parseSource(a, "try:\n    x = 1\nexcept:\n    x = 2\n");
    const node = module.module.body[0];
    try std.testing.expect(node.try_stmt.handlers[0].exc_type == null);

    const result = parseSource(a, "try:\n    x = 1\n");
    try std.testing.expectError(ParseError.InvalidTryStatement, result);
}

test "raise with and without expr" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const a = arena.allocator();

    const module = try parseSource(a, "raise ValueError(\"bad\")\n");
    const node = module.module.body[0];
    try std.testing.expect(node.* == .raise_stmt);
    try std.testing.expect(node.raise_stmt.exc.?.* == .call);

    const module2 = try parseSource(a, "raise\n");
    try std.testing.expect(module2.module.body[0].raise_stmt.exc == null);
}
